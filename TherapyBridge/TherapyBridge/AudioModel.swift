...
class AudioModel {
    ...
    var equalizerData: [Float] {
        var equalizerArray = [Float](repeating: 0.0, count: 20)
        let windowSize = fftData.count / 20
        
        for i in 0..<20 {
            let start = i * windowSize
            let end = min(start + windowSize, fftData.count)
            equalizerArray[i] = fftData[start..<end].max() ?? 0.0  // Find the maximum in the window
        }
    ...
    // MARK: Stop Method
        func stop() {
            ...
            self.audioManager?.pause()  // Pauses audio playback

            // Clear input and output blocks
    ...
    private lazy var fileReader:AudioFileReader? = {
        ...
        if let url = Bundle.main.url(forResource: "breathing", withExtension: "mp3"){
...
//        print(max)
    ...
    private func handleSpeakerQueryWithAudioFile(data: Optional<UnsafeMutablePointer<Float>>, numFrames: UInt32, numChannels: UInt32) {
        if let file = self.fileReader {
            ...
            // Set samples to input buffer, to be used for visualization
            self.inputBuffer?.addNewFloatData(data, withNumSamples: Int64(numFrames))
        }
    }
    ...
    /*
    public func startProcessingAudioFile() {
        // Set the output block to handle playback from the audio file
        self.audioManager?.outputBlock = self.handleSpeakerQueryWithAudioFile
        // Play the file
        self.fileReader?.play()
    }
    */
    ...
    public func startProcessingAudioFile() {
        ...
        // Set up a timer to regularly run `runEveryInterval` for visualization
        Timer.scheduledTimer(timeInterval: 1.0/10.0, target: self,
                             selector: #selector(self.runEveryInterval),
                             userInfo: nil,
                             repeats: true)
    }
